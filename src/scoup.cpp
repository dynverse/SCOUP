...
int main_scoup(int argc, char* argv[]){
	...
	//output
	OU.Print_gene_parameter(argv[optind+3]);
	OU.Print_cell_parameter(argv[optind+4]);
	OU.Print_ll(argv[optind+5]);
...
int main_scoup_resume(int argc, char* argv[]){
  ...
  OU.Print_cell_parameter(argv[optind+3]);
  OU.Print_gene_parameter(argv[optind+4]);
  OU.Print_ll(argv[optind+5]);
...
int main_correlation(int argc, char* argv[]){
  ...
  OU.Print_correlation(argv[5], argv[6]);
...
int main_sp(int argc, char* argv[]){
  ...
  //Prim
  PT.Prim(argv[3], argv[4]);

  // fout_pseudo_time.close();
  // fout_pca.close();
